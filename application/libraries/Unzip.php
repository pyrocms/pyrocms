...
class Unzip
{
	private $compressed_list    = array(); // List of files in the ZIP
	private $central_dir_list    = array(); // Central dir list... It's a kind of 'extra attributes' for a set of files
	private $end_of_central      = array(); // End of central dir, contains ZIP Comments
	private $info              = array();
	private $error             = array();
	...
	private $zip_signature = "\x50\x4b\x03\x04"; // local file header signature
	...
	private function _load_file_list_by_eof(&$fh, $stop_on_file=FALSE)
	{
		...
		for($x = 0; $x < 1024; $x++)
		{
			...
			if ($signature == $this->dirSignatureE)
			{
				...
				if ($this->central_dir_list) foreach($this->central_dir_list as $filename=>$details)
				{
