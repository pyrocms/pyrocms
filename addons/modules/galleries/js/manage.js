jQuery(function($){
	...
	$('select#folder_id').change(function(){
		...
		$.get(BASE_URL + 'admin/galleries/ajax_select_folder/' + $(this).val(), function(data) {
			...
			if (data) {
				...
				if (data.images) {
					...
					$.each(data.images, function(i, image){
						...
						'<li>' +
							'<img src="' + BASE_URL + 'files/thumb/' + image.id + '" alt="' + image.name + '" title="Title: ' + image.name + ' -- Caption: ' + image.description + '"' +
						'</li>'
						);
