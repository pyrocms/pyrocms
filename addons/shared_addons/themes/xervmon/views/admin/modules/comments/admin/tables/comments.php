					...
					<td class="align-center buttons buttons-small">
						<?php if ($this->settings->moderate_comments): ?>
							<?php if ($comment->is_active): ?>
								<?php echo anchor('admin/comments/unapprove/'.$comment->id, lang('buttons:deactivate'), 'class="button deactivate btn"') ?>
							<?php else: ?>
								<?php echo anchor('admin/comments/approve/'.$comment->id, lang('buttons:activate'), 'class="button activate btn"') ?>
							<?php endif ?>
						<?php endif ?>
					
						<?php echo anchor('admin/comments/edit/'.$comment->id, lang('global:edit'), 'class="button edit btn"') ?>
						<?php echo anchor('admin/comments/delete/'.$comment->id, lang('global:delete'), array('class'=>'confirm button delete btn btn-danger')) ?>
						<?php echo anchor('admin/comments/report/'.$comment->id, 'Report', array('class'=>'button edit btn')) ?>
					</td>
				</tr>
			<?php endforeach ?>
		</tbody>
