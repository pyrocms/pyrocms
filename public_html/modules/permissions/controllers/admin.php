...
class Admin extends Admin_Controller {
	...
	function __construct() {
		...
		$this->data->modules_select = array('*'=>'-- All --')+array_for_select($modules, 'slug', 'name');
	...
	function create() {
		...
		$this->data->controllers_select = array('*'=>'-- All --')+array_for_select($this->modules_m->getControllers($this->validation->module));
		$this->data->methods_select = array('*'=>'-- All --')+array_for_select($this->modules_m->getMethods($this->validation->module, $this->validation->controller));
	...
	function edit($id = 0) {
		...
		{
			$this->session->set_flashdata('error', 'This permission rule does not exist.');
		...
		// Get controllers and methods arrays for selected values to populate ajax boxes
		$this->data->controllers_select = array('*'=>'-- All --')+array_for_select($this->modules_m->getControllers($this->data->permission_rule->module));
		$this->data->methods_select = array('*'=>'-- All --')+array_for_select($this->modules_m->getMethods($this->data->permission_rule->module, $this->data->permission_rule->controller));
	...
	function delete($id = 0) {
		...
		$this->session->set_flashdata('success', 'The permission rule has been deleted.');
		redirect('admin/permissions/index');
	}
