...
class Module_Blog extends Module {
	...
	public function info()
	{
		return array(
			...
			/****
			* Comment: Peter Digby
			* Role permissions can be binary (on/off) like 'put_live'
			* Admin permissions will default to 'on', unless
			* the role name contains 'restrict' - then admin will default to 'off' (i.e unrestricted)
			* (see user module helper : group_has_role for this )
			* Array based roles can also be set which will allow switching on/off access
			* to specific groupings eg. blog categories or file folders perhaps
			* Array based roles need a name, a db table and a field name 
			* in that table (name, title, description whatever)
			* 
			****/
			'roles' => array(
				'put_live',
