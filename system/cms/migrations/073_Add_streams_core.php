...
class Migration_Add_streams_core extends CI_Migration {
	...
	public function up()
	{
		...
		$this->db->where('`stream_namespace` is null')->update($this->config->item('streams:streams_table'), array('stream_namespace' => 'streams', 'stream_prefix' => 'str_'));
		$this->db->where('`field_namespace` is null')->update($this->config->item('streams:fields_table'), array('field_namespace' => 'streams'));
	}
