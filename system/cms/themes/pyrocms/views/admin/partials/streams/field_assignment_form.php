	...
	<ul>
		...
		<li>
			...
			<div class="input">
				...
				if (substr($current_field->field_name, 0, 5) === 'lang:') {
					echo '<p><em>'.lang(substr($current_field->field_name, 5)).'</em></p>';
		...
		<?php if (property_exists($current_field, 'field_slug')): ?>
		...
		if (property_exists($current_field, 'is_required')) $is_required = ($current_field->is_required == 'yes') ? true : false;
		if (property_exists($current_field, 'is_unique')) $is_unique = ($current_field->is_unique == 'yes') ? true : false;

		?>

		<?php if (property_exists($current_field, 'is_required')): ?>
		...
		<?php if (property_exists($current_field, 'is_unique')): ?>
		...
		<?php if (property_exists($current_field, 'instructions')): ?>
		...
		<li>
			...
			<div class="input"><?php echo form_textarea('instructions', $current_field->instructions, 'id="field_instructions"');?></div>
		</li>
		...
		if( isset($current_type->custom_parameters) ):
			...
			foreach( $current_type->custom_parameters as $param ):
				...
				if (method_exists($current_type, 'param_'.$param)) {
					...
					$data['input_name']		= lang('streams:'.$current_field->field_type.'.'.$param);
