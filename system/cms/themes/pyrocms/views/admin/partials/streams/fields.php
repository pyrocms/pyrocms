<?php if (!empty($fields)): ?>
    ...
    <table class="table-list" border="0" cellspacing="0">
		...
		<?php foreach ($fields as $field):?>
			<tr>
				<td><?php echo (lang($field->field_name)) ? lang($field->field_name) : $field->field_name; ?></td>
				...
				<td class="actions">
					...
					<?php
						...
						foreach($buttons as $button)
						{
							...
							$all_buttons[] = anchor(str_replace('-field_id-', $field->id, $button['url']), lang_label($button['label']), 'class="'.$class.'"');
						}
