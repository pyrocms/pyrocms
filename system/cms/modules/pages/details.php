	...
	public function install()
	{
		...
		$default_chunks = "
			INSERT INTO ".$this->db->dbprefix('page_chunks')." (`id`, `slug`, `page_id`, `body`, `parsed`, `type`, `sort`) VALUES
			  ...
			  ('3', 'default', '3', '<p>To contact us please fill out the form below.</p> {{ contact:form name=\"text|required\" email=\"text|required|valid_email\" subject=\"text|required|max_length[100]\" message=\"textarea\" attachment=\"file|zip\" }} {{ open }} <p>Name:{{ name }}</p> <p>Email:{{ email }}</p> <p>Subject:{{ subject }}</p> <p>Message:{{ message }}</p> <p>Attach  a zip file:{{ attachment }}</p> {{ close }} {{ /contact:form }}', '', 'wysiwyg-advanced', '0');
		";
