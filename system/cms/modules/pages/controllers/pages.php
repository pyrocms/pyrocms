    ...
    public function _page($url_segments)
    {
        ...
        if (!$page or ($page->status === 'draft' and !ci()->current_user->hasAccess(array('put_live', 'edit_live')))) {
            ...
            if (!($page = Page::findByUri(404))) {
                show_error(
                    'The page you are trying to view does not exist and it also appears as if the 404 page has been deleted.'
                );
            }
        }
        ...
        elseif ($page->entry->restricted_to and $restrictedIds = $page->entry->restricted_to->lists('id')) {
            ...
            if (!$groupIds or (!in_array(1, $groupIds) and empty($matches))) {
        ...
        if ($page->rss_enabled) {
            $this->template->append_metadata(
                '<link rel="alternate" type="application/rss+xml" title="' . $page->meta_title . '" href="' . site_url(
                    uri_string() . '.rss'
                ) . '" />'
            );
        }
        ...
        $page->meta_title       = $this->parser->parse_string(
            $page->meta_title,
            array('current_user' => ci()->current_user),
            true
        );
        $page->meta_description = $this->parser->parse_string(
        ...
        $attributes = array_merge(
            $attributes,
            ($page->entry and !$_POST) ? $page->entry->getPresenter('plugin')->toArray() : array()
        );

        $html = $this->template->load_view('pages/page', array_merge(array('page' => $page), $attributes), false);

        $view = $this->parser->parse_string(
            $html,
            $page,
            true,
            false,
