...
class Page_m extends MY_Model
{
	...
	public function insert(array $input = array(), $chunks = array())
	{
		...
		if ($chunks)
		{
			...
			foreach ($chunks as $chunk)
			{
				...
				$this->file_chunk_write($id,$chunk->slug,$chunk->body);
			}
		}
	...
	*/
	...
	public function file_chunks_read($page_id,&$chunks)
	{
		/* do they have this switch on? if not just bail */
		if (!$this->settings->save_pchunks_file) return;

		/* this need to be as fast as possible! */
		$rec = $this->db->select('uri')->where('id',$page_id)->limit(1)->get('pages')->result_array();
		foreach ($chunks as $key => $chunk)
		{
			$file = FCPATH.trim($this->settings->save_pchunks_file_path,'/').'/'.$rec[0]['uri'].'/'.$chunk->slug.'.tpl';
			if (file_exists($file))
			{
				$chunk->body = file_get_contents($file);
			}
			$chunks[$key] = $chunk;
		}
	}

	public function file_chunk_write($id,$slug,$content)
	{
		/* do they have this switch on? if not just bail */
		if (!$this->settings->save_pchunks_file) return;

		$path = FCPATH.trim($this->settings->save_pchunks_file_path,'/').'/';
		/* is this path writable? */
		if (!is_really_writable($path))
		{
			/* should the model kick this out? */
			$this->session->set_flashdata('error',"Page Chunk not saved unable write to $path. Please check that this folder exists and is writable");
			return;
		}
		$rec = $this->db->select('uri')->where('id', $id)->limit(1)->get('pages')->result_array();
		$file = $path.$rec[0]['uri'].'/'.$slug.'.tpl';
	...
	public function file_chunk_delete($id)
	{
		...
		$files = glob(FCPATH.trim($this->settings->save_pchunks_file_path,'/').'/'.$rec[0]['uri'].'/*.tpl');
