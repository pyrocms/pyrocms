...
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		if (is_object(group_has_role('blog', 'restrict_categories'))) 
		{
			$this->restrict_array = array_keys((array)group_has_role('blog', 'restrict_categories'));
			$this->db->where_in ('blog_categories.id',$this->restrict_array );
		}
		...
		//this bit below(to remove categories links/buttons) doesn't work since shortcuts moved to generic theme 
		//... any ideas? I didn't see it as problematic when shortcuts were in the module itself which makes for easier 
		//individualisation of modules. There's a point where too DRY makes for bigger schlep..
	...
	public function index()
	{
		...
		//check user permissions
			...
			$base_where = !empty($this->restrict_array) ? $base_where + array('categories' => $this->restrict_array) : $base_where;
	...
	public function create()
	{
		...
		if ($this->form_validation->run())
		{
			...
			if (!empty($this->restrict_array) AND !in_array($this->input->post('category_id'), $this->restrict_array))
			{
