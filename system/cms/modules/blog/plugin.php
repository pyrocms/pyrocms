...
class Plugin_Blog extends Plugin
{
    ...
    public function _self_doc()
    {
        ...
        $info = array(
            'posts'       => array(
                ...
                'single'      => false,
                // single tag or double tag (tag pair)
                'double'      => true,
                'variables'   => 'category_title|category_slug|author_name|title|slug|url|category_id|intro|body|parsed|created_at|updated_on|count',
                // the variables available inside the double tags
    ...
    public function posts()
    {
        ...
        return $this->posts
            ->where('status', 'live')
            ->orderBy($this->attribute('order_by', 'created_at'), $this->attribute('sort', 'desc'))
            ->skip($this->attribute('offset'))
            ->take($this->attribute('limit'))
            ->get()->toArray();
