...
class Chunks extends FieldTypeAbstract
{
    ...
    public function formInput()
    {
        ...
        if (isset($this->entry->id)) {
            $data['chunks'] = $this->CI->page_chunk_m->get_many_by('page_id', $this->entry->id);
        } else {
            $data['chunks'] = array(
                array(
                    'slug'  => 'default',
        ...
        if ($_POST) {
            ...
            $chunk_slugs   = $this->CI->input->post('chunk_slug') ? array_values(
                $this->CI->input->post('chunk_slug')
            ) : array();
            $chunk_classes = $this->CI->input->post('chunk_class') ? array_values(
                $this->CI->input->post('chunk_class')
            ) : array();
            $chunk_bodies  = $this->CI->input->post('chunk_body') ? array_values(
                $this->CI->input->post('chunk_body')
            ) : array();
            $chunk_types   = $this->CI->input->post('chunk_type') ? array_values(
                $this->CI->input->post('chunk_type')
            ) : array();
