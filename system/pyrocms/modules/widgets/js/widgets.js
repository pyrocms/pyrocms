	...
	pyro.widgets = {
		...
		init: function(){
			$.extend(true, pyro.widgets, {
				...
				sort_options	: {
					...
					update: function(){
						...
						$.post(BASE_URI + 'widgets/ajax/update_order', { order: order.join(',') });
					}
				}
			});
			...
			$('input[name="title"]').live('keyup', $.debounce(350, function(e){
				...
				$.post(BASE_URI + 'ajax/url_title', { title : $(this).val() }, function(slug){
			...
			$('#edit-area-box form').live('submit', function(e){
				...
				$.post(BASE_URI + 'widgets/ajax/edit_widget_area', form_data, function(data){
			...
			$('#add-instance-box form').live('submit', function(e){
				...
				$.post(BASE_URI + 'widgets/ajax/add_widget_instance', form.serialize(), function(data){
			...
			$('#edit-instance-box form').live('submit', function(e){
				...
				$.post(BASE_URI + 'widgets/ajax/edit_widget_instance', form.serialize(), function(data){
		...
		refresh_lists: function(){
			$('.widget-list').each(function(){
				...
				$(this).load(BASE_URI + 'widgets/ajax/list_widgets/' + widget_area_slug, function(){
		...
		set_droppable: function(){
			$("#widget-areas .accordion-content").droppable({
				...
				drop : function(event, ui){
					...
					$.post(BASE_URI + 'widgets/ajax/add_widget_instance_form', { area_slug: area_slug, widget_slug: widget_slug}, function(data){
						if (data.status == 'success')
						{
