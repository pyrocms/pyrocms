(function($) {
	$(function(){
		...
		$('input[name="title"]', form).keyup($.debounce(300, function(){
			...
			$.post(BASE_URI + 'ajax/url_title', { title : $(this).val() }, function(new_slug){
				slug.val( new_slug );
			});
		}));
		...
		$('#btn_compare_revisions').click(function() {
			...
			$.colorbox({
				width: "85%",
				height: "85%",
				href: BASE_URI + 'admin/pages/compare/' + first + '/' + second
			});
		});
		...
		$('#btn_preview_revision').click(function() {
			...
			$.colorbox({
				width: "85%",
				height: "85%",
				href: BASE_URI + 'admin/pages/preview_revision/' + revision
			})
		});
