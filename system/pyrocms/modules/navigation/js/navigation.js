(function($) {
	...
	$(function() {
		...
		$('#nav-create button:submit').live('click', function(e){
			e.preventDefault();
			$.post(BASE_URI + 'admin/navigation/create', $('#nav-create').serialize(), function(message){
		...
		$('#nav-edit button:submit').live('click', function(e){
			e.preventDefault();
			$.post(BASE_URI + 'admin/navigation/edit/' + $('input[name="link_id"]').val(), $('#nav-edit').serialize(), function(message){
		...
		$('#link-list li a').live('click', function()
		{
			...
			$('div#link-details.group-'+ id +'').load(BASE_URI + 'admin/navigation/ajax_link_details/' + link_id, '', function(){
				$('div#link-details.group-'+ id +'').fadeIn();
			});
			return false;
		});
		...
		var update_tree = function() {
			...
			{
				// Load the details box in
				$('div#link-details').load(BASE_URI + 'admin/navigation/ajax_link_details/' + $('#link-details #link-id').val());
			}
		}
		update_tree();
		...
		$('ol.sortable').nestedSortable({
			...
			stop: function(event, ui) {
				...
				$.post(BASE_URI + 'admin/navigation/order', { 'order': order, 'group': group } );
			}
		});
	});
