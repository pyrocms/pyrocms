<?php defined('BASEPATH') OR exit('No direct script access allowed');

$lang['permissions.group']				= 'Grupo';
$lang['permissions.roles']                  = 'Roles'; #translate
$lang['permissions.edit']				= 'Editar permissões';

$lang['permissions.introduction'] 		= 'Você pode criar permissões personalidadas para usuários diferentes
	atribuindo-os a grupos de utilizadores > Gerenciar área de usuários. Então você poderá editar as permissões de cada
	grupo e controlar quais modulos e "papeis" um grupo pode ter.';

$lang['permissions.message_group_saved'] = 'As permissões para este grupo foram salvas.';

/* End of file permissions_lang.php */
/* Location: ./system/pyrocms/modules/permissions/language/brazilian/permissions_lang.php */